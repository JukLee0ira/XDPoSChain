...
func (st *StateTransition) buyGas() error {
	...
	if balanceTokenFee == nil {
		...
		if have, want := st.state.GetBalance(st.msg.From()), balanceCheck; have.Cmp(uint256.MustFromBig(want)) < 0 {
	...
	if balanceTokenFee == nil {
		st.state.SubBalance(st.msg.From(), uint256.MustFromBig(mgval))
	}
	return nil
}
...
func (st *StateTransition) TransitionDb(owner common.Address) (*ExecutionResult, error) {
	...
	if contractCreation {
		ret, _, st.gas, vmerr = st.evm.Create(sender, st.data, st.gas, uint256.MustFromBig(st.value))
	} else {
		// Increment the nonce for the next transaction
		st.state.SetNonce(sender.Address(), st.state.GetNonce(sender.Address())+1)
		ret, st.gas, vmerr = st.evm.Call(sender, st.to().Address(), st.data, st.gas, uint256.MustFromBig(st.value))
	}
	if !eip3529 {
...
func (st *StateTransition) refundGas(refundQuotient uint64) {
	...
	if balanceTokenFee == nil {
		...
		st.state.AddBalance(from.Address(), uint256.MustFromBig(remaining))
	}
