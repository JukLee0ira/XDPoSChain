...
func TransactionToMessage(tx *types.Transaction, s types.Signer, balanceFee, blockNumber, baseFee *big.Int) (*Message, error) {
	...
	if balanceFee != nil {
		...
		msg.GasPrice = msg.GasPrice.Add(msg.GasPrice, baseFee)
		if msg.GasPrice.Cmp(msg.GasPrice) > 0 {
			msg.GasPrice.Set(msg.GasPrice)
		}
	}
...
func (st *StateTransition) preCheck() error {
	...
	if st.evm.ChainConfig().IsEIP1559(st.evm.Context.BlockNumber) {
		...
		if !st.evm.Config.NoBaseFee || msg.GasFeeCap.BitLen() > 0 || msg.GasTipCap.BitLen() > 0 {
			...
			if (msg.To == nil || *msg.To != common.RandomizeSMCBinary) && msg.GasFeeCap.Cmp(st.evm.Context.BaseFee) < 0 {
				return fmt.Errorf("%w: address %v, maxFeePerGas: %s baseFee: %s", ErrFeeCapTooLow,
					msg.From.Hex(), st.msg.GasFeeCap, st.evm.Context.BaseFee)
			}
		}
	}
	return st.buyGas()
...
func (st *StateTransition) TransitionDb(owner common.Address) (*ExecutionResult, error) {
	...
	gas, err := IntrinsicGas(msg.Data, msg.AccessList, contractCreation, homestead, rules.IsEIP1559)
	if err != nil {
		return nil, err
	}
	...
	if st.evm.Context.BlockNumber.Cmp(common.TIPTRC21Fee) > 0 {
		if (owner != common.Address{}) {
			st.state.AddBalance(owner, new(big.Int).Mul(new(big.Int).SetUint64(st.gasUsed()), st.msg.GasPrice))
		}
	} else {
		effectiveTip := st.msg.GasPrice
		if st.evm.ChainConfig().IsEIP1559(st.evm.Context.BlockNumber) {
			effectiveTip = new(big.Int).Sub(msg.GasTipCap, st.evm.Context.BaseFee)
